...
def fetch_img():
    ...
    res = predicta(img1)
    fini_time = datetime.datetime.now()
    print("finish predict, concume: ", (fini_time - begin_time).microseconds)
    
    #res = predicta(img1)

    if ((thetime - (datetime.datetime.strptime(tmp_img["timestamp"], '%d:%m:%Y:%H:%M:%S'))).seconds):
        upload_response(res)
...
def hello_world():
    ...
    # return render_template('index.html',img_stream=img_stream)
        # index1 是逐帧
    return  render_template("index1.html", img_stream = img_stream,situation = "Begin")

@app.route('/', methods=['POST','GET'])  # for: index1 逐帧
def update_pic():
    if request.method == "POST":
        if request.form["submit_button"] == "Manual mode":
            img_s, resp = fetch_img()
            return render_template('index1.html', img_stream=img_s, situation = resp)
...
mongodbUri = 'mongodb://root:root@127.0.0.1:27017/admin'
# user1，myadmin
# user2，myadmin
# user3，myadmin
...
app.run(host="10.0.12.11",port=8000)
#app.run()
